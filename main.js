...
const qrCodeSuccessCallback = (decodedText, decodedResult) => {
    ...
    if ((urlRegex.test(decodedText) == true && decodedText.includes(url) == true) )  {
        ...
        notifyelementsuccess.display="none";

        var newDecodedText = decodedText.substr(24, 6);
        // console.log("new decodedText", newDecodedText);
        var raw = JSON.stringify({
            "DataSource": "[API].[ReceiptQRCode_GetID]",
            "ID": newDecodedText
        });
        ...
        async function GetAPI() {
            ...
            document.getElementById('expiredate').textContent = Data[0].ExpireDate;
            document.getElementById('mass').textContent = Data[0].Mass;
            document.getElementById('origin').textContent = Data[0].Origin;
            document.getElementById('producer').textContent = Data[0].Producer;
            document.getElementById('supplier').textContent = Data[0].Supplier;
            document.getElementById('categoriesname').textContent = Data[0].CategoriesName;
            document.getElementById('ingredients').textContent = Data[0].Ingredients;
            document.getElementById('price').textContent = Data[0].Price + " đ";
